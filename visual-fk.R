...
d_df <- tibble(name = as_factor(c("Initial", "G", "Updated", "M", "Predictive")),
       dist = c(d_initial, NA, d_update, NA, d_predict),
       type = "analytical"
       )
...
d_plot <- ggplot() + ggdist::stat_slab(aes(y = name, dist = dist), data = d_df) +
  ggdist::stat_slab(aes(y = "G", dist = potfun), fill = NA, colour = "red") + 
  ...
  geom_text(aes(x = 9, y = "G"), label = "G[0](x)==phi(textstyle(frac(x-1,sqrt(2))))", parse = T, size=anno_size, nudge_y = 0.45, colour = "red") +
  ...
  geom_text(aes(x = 7, y = "M"), label = "M[1](x[0],.)==N(x[0]-2, 1)", parse = T, size=anno_size, nudge_y = 0.45, colour = "blue")
...
d_t_df <- tibble(name = as_factor(c("Initial", "G", "Updated", "M", "Predictive")),
               dist = c(d_t_initial, NA, d_t_update, NA, d_t_predict),
               type = "analytical"
)
...
d_t_plot <- ggplot() + ggdist::stat_slab(aes(y = name, dist = dist), data = d_t_df) +
  ggdist::stat_slab(aes(y = "G", dist = t_potfun), fill = NA, colour = "red") + 
  ggdist::stat_slab(aes(y = "M", dist = kerdist), fill = NA, colour = "blue", normalize = "all", linetype = "dashed") + 
  theme_bw() + 
  scale_y_discrete(limits = rev) +
  theme(axis.title.x=element_blank(),
  ...
  theme(panel.grid.minor = element_blank(), panel.grid.major.x = element_blank(), panel.border = element_blank()) +
  geom_text(aes(x = 9, y = "G"), label = "{G[0]^psi}(x)==1", parse = T, size=anno_size, nudge_y = 0.45, colour = "red") +
  ...
  geom_text(aes(x = 7, y = "M"), label = "{M[1]^psi}(x[0],.)==N(x[0]-2, 1)", parse = T, size=anno_size, nudge_y = 0.45, colour = "blue")
