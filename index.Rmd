...
date: "CoSInES x Bayes4Health Seminar, Nov '22"
output:
...
--
Use to express $\hat{\eta}_{p-1}^{\omega}(M_p(\omega_p))$ without $\omega$-twisting then some algebra
...
- Define $(\tilde{M}_{0:n}, \tilde{G}_{0:n})$, equivalent FK model on augmented state-space $(X_p, U_p)$ to represent $(M_{0:n}, \bar G_{0:n})$

- In general, the asymptotic variance is

$$\sigma^2_n(\varphi) = \sum_{p=0}^{n} v_{p,n}(\varphi),\quad \text{where}\quad 
v_{p,n}(\varphi) = \frac{\gamma_p(1)\gamma_p\left(Q_{p,n}(\varphi)^2\right)}{\gamma_n(1)^2} - \eta_{n}(\varphi)^2$$

- Relate $\tilde{Q}_{p,n}$ to $Q_{p,n}$ using $\tilde{G}_p(x_p,u_p) / G_p(x_p)$

- Then relate $\tilde{\gamma}_p\left(\tilde{Q}_{p,n}(\varphi)^2\right)$ to $\gamma_p\left(Q_{p,n}(\varphi)^2\right)$ using $s_p(x_p) = \mathbb{E}[\tilde{G}_p(x_p, U_p)/G_p(x_p)]^2$.
